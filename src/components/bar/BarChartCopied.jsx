...
const BarChartCopied = props => {
  ...
  const selectedData = barData.barData.filter(
    datum => datum.Vendor === selectedGroup
  );
...
//   );
  ...
  const bars = selectedData.map((datum, index) => (
    <Bar
      ...
      y={0}
      width={7.8}
      height={(barChartHeight - y(datum.Volume)) / 2}
      colour={barColour}
      count={datum.Volume}
    />
  ));
